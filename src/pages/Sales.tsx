...
export default function Sales() {
  ...
  return <div className="flex h-full min-h-0 flex-col gap-3 overflow-y-auto xl:overflow-hidden">
    <div className="flex shrink-0 items-center justify-between gap-3"><div><h1 className="text-xl font-bold text-surface-900">Frente de Vendas</h1><p className={`text-xs ${operationStatus ? 'font-medium text-brand-600' : 'text-surface-500'}`}>{operationStatus || 'Venda, baixa no estoque e emissão automática de comprovante'}</p></div><button className="btn-secondary flex shrink-0 items-center gap-2" onClick={() => setSalesOpen(true)}><History size={16}/>Vendas <span className="rounded-full bg-surface-100 px-1.5 text-xs">{summary.completedSales}</span></button></div>
    ...
    {canCreate && <div className="grid min-h-0 flex-1 grid-cols-1 gap-4 xl:grid-cols-[minmax(0,1fr)_390px]">
      ...
      <aside className="card flex min-h-0 flex-col overflow-hidden">
        <div className="flex shrink-0 items-center gap-2 border-b border-surface-100 px-3 py-2"><ShoppingCart size={18}/><h2 className="font-semibold">Carrinho ({cart.length})</h2></div>
        <div className="min-h-0 flex-1 divide-y overflow-y-auto">{cart.map((item) => <div key={item.id} className="p-2.5">
        ...
        <div className="shrink-0 space-y-2 border-t bg-surface-50 p-2.5">
          <div className="grid grid-cols-2 gap-2"><div><label className="mb-1 block text-[11px]">Pagamento</label><select className="w-full" value={paymentMethod} disabled={!!selectedCommand} onChange={(e) => setPaymentMethod(e.target.value)}>{paymentMethods.filter(([value]) => value !== 'COMANDA' || selectedCommand).map(([value,label]) => <option key={value} value={value}>{label}</option>)}</select></div><div><label className="mb-1 block text-[11px]">Desconto</label><input className="w-full" type="number" min="0" max={subtotal} step="0.01" value={discount || ''} onChange={(e) => setDiscount(Number(e.target.value))}/></div></div>
          ...
          <div className="flex items-end justify-between gap-3"><div className="min-w-0"><p className="text-[11px] text-surface-500">Subtotal {money(subtotal)}</p><p className="text-lg font-bold leading-tight">Total <span className="text-brand-600">{money(total)}</span></p></div><button className="btn-primary min-w-[155px] shrink-0 disabled:opacity-50" disabled={saving || !cart.length} onClick={finishSale}>{saving ? 'Processando...' : 'Concluir venda'}</button></div>
        </div>
      </aside>
    </div>}
